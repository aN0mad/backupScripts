...
Get-Item -LiteralPath $location -Force | Where{$_.LastWriteTime -ge (Get-Date).AddHours(-3)} | %{$_.FullName} {Copy-Item -Force -Recurse $_ -Destination ($Destination + "\" + $_.Name) -Container}